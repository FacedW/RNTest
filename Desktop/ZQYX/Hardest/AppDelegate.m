...
@property (nonatomic,strong) IPConfigModel *configModel;
@end
...
- (void)getIPConfigData{
    ...
    [HttpConnectt HttpWithURL:@"https://operate.liulianggame.com/operate/app/nodes" paramDic:paramsMutDic completion:^(NSDictionary * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        if (error == nil && [data[@"code"] integerValue] == 0) {
            ...
            _configModel = [IPConfigModel modelWithJSON:successInfoDic];
        }else{
...
- (void)doChangeByConfig{
    NSInteger styleNum = [_configModel.projectStatus integerValue];//项目状态 1A 2B 3 WAP
    switch (styleNum) {
        case 3:
        {
            //web
            [self changeRootViewcontroller:_configModel];
        }
            break;
